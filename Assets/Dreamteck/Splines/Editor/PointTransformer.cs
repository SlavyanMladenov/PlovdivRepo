...
namespace Dreamteck.Splines {
    public class PointTransformer
    {
        ...
        public PointTransformer(SplinePoint[] points, List<int> selection, Transform local)
        {
        ...
        public PointTransformer(SplinePoint[] points, List<int> selection)
        {
            localTransform = null;
            selected = selection.ToArray();
            originalPoints = points;
        ...
        public SplinePoint[] GetRotated(bool rotateNormal, bool rotateTangents)
        {
            ...
            if (localTransform != null) rot = rotation * Quaternion.Inverse(localTransform.rotation);
            for (int i = 0; i < selected.Length; i++)
            {
