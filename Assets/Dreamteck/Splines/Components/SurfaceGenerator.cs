...
namespace Dreamteck.Splines
{
    ...
    public class SurfaceGenerator : SplineMeshGeneration
    {
        public float expand
        {
            get { return _expand; }
        ...
        public void Generate()
        {
            ...
            {
                tsMesh.uv[i].x = Mathf.InverseLerp(max.x, min.x, projectedVerts[i].x) * _uvScale.x - _uvScale.x * 0.5f + _uvOffset.x + 0.5f;
                tsMesh.uv[i].y = Mathf.InverseLerp(min.y, max.y, projectedVerts[i].y) * _uvScale.y - _uvScale.y * 0.5f + _uvOffset.y + 0.5f;
            }
